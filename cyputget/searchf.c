...
#if 0
/*  Debug function printing filename in FCB
 */
VOID prtfcbfn(dbghdr, dev, pf)
    TEXT *dbghdr;   /* Text to identify printout */
    COUNT dev;      /* Device number (Drive) */
    BDOSFCB *pf;    /* Pointer to FCB */
    {
    putfmt("%p ", dbghdr);
    putfmt("device = %s\n", dev);
    putfmt("FCB file drive = %i, name: \"%b\", type = \"%b\"\n",
       pf->dr,  pf->f, 8, pf->t, 3);
    }

/*  Debug function printing directory entry
 */
VOID prtdirent(disk, entno)
    COUNT disk;     /* Disk number (device) */
    BYTES entno;    /* Entry # in directory */
    {
    IMPORT TEXT diskbuf[128];
    DIRENT *entptr;

    putfmt("Disk = %a: ", 'A' + disk);
    entptr = &diskbuf[entno * 32];
    putfmt("Directiry entry #  = %i, name: \"%b\", type = \"%b\"\n",
       entno,  entptr->f, 8, entptr->e, 3);
    }
#endif
...
COUNT searchf(s, sav, maxfnam)
    ...
    ffnmlen = ffnm(pfnm, dev, cpmres);
    sav[fnames] = pfnm;
    pfnm += ffnmlen;
    
    for (fnames = 1; fnames < maxfnam; fnames++)
        {
