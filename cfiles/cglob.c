/*  CGLOB.C - CP/M Filename glob function to handle wildcards
 *
 ...
 *  Hastily Cobbled Together 2021, 2024 by Hans-Ake Lund
 */
#include <std.h>
#include <cpm.h>

/* Max number of file names to match */
#define MAXFILENM 128

/* Disk read write buffer size, always 128 bytes */
#define DSKBUFSIZ 128
...
LOCAL BDOSFCB srchfcb;                      /* FCB used to search matching filenames */
LOCAL TEXT diskbuf[DSKBUFSIZ] = {0};        /* disk read buffer */
LOCAL TINY curdev = {NODEV};                /* currently selected device */
...
LOCAL COUNT ffnm(ifnm, disk, entno)
    ...
    *fnm++ = ':';
    fnmlen = 2;
    entptr = &diskbuf[entno * DIRENTSIZ];
